...
const generate = function (seed) {
  ...
  if (ordinary) {
    ...
    if (division && !charge && !usedPattern && P(0.5) && ordinary !== "bordure" && ordinary !== "orle") {
      if (P(0.8)) coa.ordinaries[0].divided = "counter"; // 40%
      else if (P(0.6)) coa.ordinaries[0].divided = "field"; // 6%
  ...
  if (charge) {
    ...
    if (p === "ABCDEFGHIKL" && P(0.95)) {
      ...
      } else if (allowCounter && P(0.4)) coa.charges[0].divided = "counter"; // counterchanged, 40%
